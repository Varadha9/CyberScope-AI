...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS devices (
            ...
            ip TEXT, mac TEXT, status TEXT DEFAULT 'active',
            last_seen TEXT
        )
    """)
    c.execute("""
...
def save_device(ip, mac):
    ...
    if c.fetchone():
        c.execute("UPDATE devices SET last_seen=? WHERE ip=?", (datetime.now().isoformat(), ip))
    else:
...
def get_all_alerts():
    conn = sqlite3.connect(DB)
    rows = conn.execute("SELECT alert, severity, timestamp FROM alerts ORDER BY id DESC LIMIT 50").fetchall()
    conn.close()
